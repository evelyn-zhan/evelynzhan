    ...
    return (
        <header className="px-3.5 lg:px-16 py-5 lg:py-10">
            <nav className="flex flex-row justify-end items-center gap-x-7 lg:gap-x-10 text-lg lg:text-xl font-medium">
                <Link to="/" className={`${currentPath.pathname === "/" || currentPath.pathname === "/about/overview" || currentPath.pathname === "/about/education" ? (darkMode ? 'text-primary' : 'text-darker') : ''} hover:font-bold transition-all ease-in-out duration-600`}>About</Link>
                ...
                {
                    darkMode === false
                    ?   <div className="cursor-pointer px-2 lg:px-2.5 py-1 lg:py-1.5 bg-darker rounded-md border-solid border-[1.5px] border-darker text-white hover:bg-white hover:text-darker transition-all ease-in-out duration-600" onClick={changeMode}>
